...
class PayUPayments{
	...
	public static function doPayment($parameters, $transactionType, $lang){
		...
		if (array_key_exists(PayUParameters::TOKEN_ID,$parameters)) {
		...
		}else if($paymentMethodParameter != null && PaymentMethods::isAllowedCashPaymentMethod($paymentMethodParameter)){
			$requiredCash = array(
					PayUParameters::PAYER_NAME,
		...
		}else if($paymentMethodParameter != null && PaymentMethods::BOLETO_BANCARIO == $paymentMethodParameter) {
		...
		}else if($paymentMethodParameter != null) {
			...
			throw new InvalidArgumentException
						(sprintf("The payment method parameter [%s] was not found",PayUParameters::PAYMENT_METHOD));
		}
